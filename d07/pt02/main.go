...
func process(nums []int, target, current, idx int) bool {
	...
	return process(nums, target, current+nums[idx], idx+1) || process(nums, target, current*nums[idx], idx+1) || process(nums, target, concat(current, nums[idx]), idx+1)
}
